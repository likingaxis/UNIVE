...
var PenHighlighterPlugin = class extends import_obsidian.Plugin {
  ...
  buildKeymapExtension() {
    const formatSelection = (view, delimiter) => {
  ...
  showFormattingMenu(editor, selectedText, x, y) {
    ...
    menu.addItem((item) => {
      item.setTitle("1. *ciao* (Corsivo)").setIcon("italic-glyph").onClick(() => {
        editor.replaceSelection(`*${selectedText}*`);
      });
    });
    menu.addItem((item) => {
      item.setTitle("2. **ciao** (Grassetto)").setIcon("bold-glyph").onClick(() => {
        editor.replaceSelection(`**${selectedText}**`);
      });
    });
    menu.addItem((item) => {
...
var PenHighlighterSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    new import_obsidian.Setting(containerEl).setName("Richiedi l'uso della penna per il Menu").setDesc("Se attivato, il menu a comparsa apparir\xE0 SOLO quando usi la penna. Se disattivato, apparir\xE0 anche selezionando il testo col dito o col mouse.").addToggle((toggle) => toggle.setValue(this.plugin.settings.requirePen).onChange(async (value) => {
      this.plugin.settings.requirePen = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("Scorciatoie numeriche (1, 2, 3)").setDesc("Permette di formattare il testo selezionato premendo 1 (*), 2 (**), 3 (***) sulla tastiera. Se non c'\xE8 testo selezionato, i numeri vengono digitati normalmente.").addToggle((toggle) => toggle.setValue(this.plugin.settings.enableShortcuts).onChange(async (value) => {
